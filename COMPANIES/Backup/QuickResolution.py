...
class Ui(QtWidgets.QWidget):
    ...
    def AddAuthorized(self,label,button):
        if button.text() not in self.AuthorizedContainer:
            self.AuthorizedContainer.append(button.text())
            self.AuthTracker[label].append(self.AuthorizationsWindow.BTGroup.id(button))
        elif button.text() in self.AuthorizedContainer:
            self.AuthorizedContainer.remove(int(button.objectName().replace('cbtn','')))
            self.AuthTracker[label].remove(self.AuthorizationsWindow.BTGroup.id(button))
        else:
            None
