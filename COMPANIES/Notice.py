...
from functions.generateDocuments import Notice as ntc
...
class Ui(QtWidgets.QWidget):
    def __init__(self,CoName):
        ...
        if HasAgenda:
            ...
            CompanyList=['']
            self.fy.setText(self.findfinyear(datetime.datetime.now().date()))
            self.AgendaNoteList = {}
    ...
    ##        self.AgendaTable.setCellWidget(0, 0, comboBox)
            ...
            self.Committe.clicked.connect(self.enableCommittee)
            self.BoardMeeting.clicked.connect(self.enableCommittee)
            self.FBM.clicked.connect(self.enableCommittee)
            self.EGM.clicked.connect(self.enableCommittee)
            self.AGM.clicked.connect(self.enableCommittee)
            self.show()
        else:
            WarnUser = QtWidgets.QMessageBox()
            WarnUser.setText("No predefined agendas found. If yo are an Admin add agendas. If you are not the admin, ask admin to add Agendas")
            WarnUser.show()

    def enableCommittee(self):
        sender =(self.sender())
        BtButton = sender.objectName()
        if BtButton=='Committe':
    ...
    def GetSignatoriesList(self):
        ...
        self.DirectorNames = self.cur.execute(f'SELECT director_name, director_gender, Designation from Signatories WHERE company_cin = {repr(self.CINNum)}').fetchall()
        self.DirectorList=['']
        self.addressees = []
        for item in self.DirectorNames:
            self.DirectorList.append(item[0])
            self.addressees.append([item[0],item[1],item[2]])
        self.Signedby.clear()
    ...
    def expandAgendas(self):
        ...
        for x in range(4):
            ...
            if x==0:
               item.setText("Appointment of Chairman")
               item = QtWidgets.QTableWidgetItem()
    ...
    def generateNoticeDoc(self):
        ...
        self.conn = sqlite3.connect(self.dbfilepath)
        self.cur = self.conn.cursor()
        Lthead = self.cur.execute(f'SELECT company_registered_address,company_email_id, company_phone from Masterdata WHERE company_name = "{self.currentselection}"').fetchall()[0]
        Notice=ntc.createDoc()
        Notice.letterhead(self.currentselection,Lthead[0],self.CINNum, Lthead[1],Lthead[2],self.isLetterhead.isChecked())
        Notice.Noticetitle()
        Notice.NoticeTo(self.addressees)
        SignatoriesList=[]
        ...
        if self.BoardMeeting.isChecked():
            ...
            MeetShrt = 'BM_Notice_'
        elif self.Committe.isChecked():
            MeetingType=f'Meeting of the members of {self.commiteeName.text()}'
            MeetingName = 'Committee Meeting'
            MeetingNo = self.MeetingNo.value()
            MeetShrt = 'Committee_Notice_'
        elif self.AGM.isChecked():
            MeetingType='Annual General Meeting'
            MeetingName = 'Annual General Meeting'
            MeetingNo = self.MeetingNo.value()
            MeetShrt = 'AGM_Notice_'
        elif self.EGM.isChecked():
            MeetingType='Extra-Ordinary General Meeting'
            MeetingName = 'Extra Ordinary General Meeting'
            MeetingNo = ''
            MeetShrt = 'EGM_Notice_'
        elif self.FBM.isChecked():
            MeetingType='First Board Meeting'
            MeetingName = 'Board Meeting'
            MeetingNo = ''
            MeetShrt = 'BM_Notice_'
        RegOffSwitch = Lthead[0] if self.VenueSelect.text().lower()=='registered office' else self.VenueSelect.text()
        isRegOffi = True if self.VenueSelect.text().lower()=='registered office' else False
        Notice.NoticeBody(MeetingNo,MeetingType,MeetingName,self.fy.text(),self.TMZ.currentText(),self.currentselection,RegOffSwitch,self.DateSelection.date().toPython(),self.TimeSelect.time().toPython(),
                          isRegOffi,Agenda=SignatoriesList)    
        Path(os.path.join(self.Config['Home'],self.currentselection,self.findfinyear(self.DateSelection.date()),MeetingName,datetime.datetime.strftime(datetime.datetime.strptime(self.DateSelection.date().toString(),'%a %b %d %Y').date(),'%d.%m.%Y'))).mkdir(parents=True, exist_ok=True)
        ...
        self.filepath = os.path.join(self.Config['Home'],self.currentselection,self.findfinyear(self.DateSelection.date()),MeetingName,datetime.datetime.strftime(self.DateSelection.date().toPython(),'%d.%m.%Y'),MeetShrt+datetime.datetime.strftime(datetime.datetime.strptime(self.DateSelection.date().toString(),'%a %b %d %Y').date(),'%d_%m_%Y'))
        savePath = os.path.join(self.currentselection,self.findfinyear(self.DateSelection.date()),MeetingName,datetime.datetime.strftime(self.DateSelection.date().toPython(),'%d.%m.%Y'),MeetShrt+datetime.datetime.strftime(datetime.datetime.strptime(self.DateSelection.date().toString(),'%a %b %d %Y').date(),'%d_%m_%Y'))
        Notice.saveDoc(self.filepath)       
        ...
        MinutesDb = [self.CINNum,datetime.datetime.strftime(self.DateSelection.date().toPython(),'%d-%m-%Y'),MeetingName,self.fy.text(),CurrentUserName,'',savePath,'']
        db.Minutes(MinutesDb)
