...
from COMPANIES import ClientManager, AddCommittee
from COMPANIES import Notice, Minutes
from REGISTERS import MGT1
import pickle
...
class Ui(QtWidgets.QWidget):
    def __init__(self,DirectorDIN):
        ...
        self.Sidebar.layout().addWidget(self.createButton('Committee Manager', self.committeeManager))
    ...
    def committeeManager(self):
        if os.path.isfile(self.committeFilePath):
            self.conn = sqlite3.connect(self.committeFilePath)
            self.cur = self.conn.cursor()
            try:
                self.Committees = self.cur.execute(f'SELECT * from committee WHERE company_cin = "{self.CIN}"').fetchall()
                hasCommittee = True
            except sqlite3.OperationalError:
                hasCommittee = False
            if not hasCommittee or len(self.Committees)==0:
                ...
                label.setMinimumSize(QtCore.QSize(300, 30))
                label.setMaximumSize(QtCore.QSize(1667725, 30))
                label.setAlignment(QtCore.Qt.AlignCenter|QtCore.Qt.AlignVCenter)
                font = QtGui.QFont()
                font.setFamily("Georgia")
                font.setPointSize(12)
                label.setFont(font)
                label.setText("The Company has no Committees")
                layout.addWidget(label)
                buttonWidget = QtWidgets.QWidget()
                buttonLayout = QtWidgets.QHBoxLayout()
                buttonLayout.addItem(QtWidgets.QSpacerItem(40, 10, QtWidgets.QSizePolicy.Expanding, QtWidgets.QSizePolicy.Minimum))
                ...
                button.setMinimumSize(QtCore.QSize(120, 30))
                button.setMaximumSize(QtCore.QSize(120, 30))
                button.setText("Add Committee")
                button.clicked.connect(self.addCommitte)
                font = QtGui.QFont()
                font.setFamily("Georgia")
        ...
        "border-radius:10")
                button.setObjectName("addCommitteebutton_blank")
                ...
                Widget = QtWidgets.QWidget()
                pyside_dynamic.loadUi('Resources/ui/CommitteeManager.ui',Widget)
                Widget.CommitteeTable.setColumnWidth(0, 150)
                Widget.CommitteeTable.setColumnWidth(1, 300)
                Widget.CommitteeTable.setColumnWidth(2, 200)
                Widget.CommitteeTable.setColumnWidth(3, 200)
                Widget.CommitteeTable.setColumnWidth(4, 200)
                Widget.CommitteeTable.setContextMenuPolicy(QtCore.Qt.CustomContextMenu)
                Widget.CommitteeTable.customContextMenuRequested.connect(self.CommitteecontextMenu)
                Widget.CommitteeTable.cellDoubleClicked.connect(self.openCommittee)
                Widget.addCommittee.clicked.connect(self.addCommitte)
                Widget.CommitteeTable.setRowCount(len(self.Committees))
                for ROW,Committee in enumerate(self.Committees):
                    Data = self.Committees[ROW][1:]
                    for COL in range(len(Data)):
                        item = QtWidgets.QTableWidgetItem()
                        Widget.CommitteeTable.setItem(ROW, COL, item)
                        item.setText(str(Data[COL]))
                        item.setFlags(QtCore.Qt.ItemIsEnabled)
                    viewPolicy = QtWidgets.QToolButton()
                    icon = QtGui.QIcon()
                    icon.addPixmap(QtGui.QPixmap(QtGui.QImage("Resources/Icon/review.svg")), QtGui.QIcon.Normal)
                    viewPolicy.setIcon(icon)
                    viewPolicy.setObjectName("viewPolicy")
                    viewPolicy.setToolTip("Show Policy")
                    if Data[-1]!='':
                        viewPolicy.clicked.connect(self.viewPolicyDoc)
                    else:
                        viewPolicy.setEnabled(False)
                    Widget.CommitteeTable.setCellWidget(ROW, Widget.CommitteeTable.columnCount()-1, viewPolicy)
                Widget.CommitteeTable.setWordWrap(True)
                Widget.CommitteeTable.resizeRowsToContents();
    ...
    "border-radius:10")
            ...
            layout.addItem(QtWidgets.QSpacerItem(20, 40, QtWidgets.QSizePolicy.Minimum, QtWidgets.QSizePolicy.Expanding))
            Widget.setLayout(layout)
        clearLayout(self.DisplayArea.layout())
        self.DisplayArea.layout().addWidget(Widget,*(0,0))
        
    def CommitteecontextMenu(self, pos):
        sender = self.sender()
        x,y = pos.x(), pos.y()
        ...
        delete = menu.addAction("Delete Committee")
        action = menu.exec_(sender.viewport().mapToGlobal(pos))
        self.deleteCommittee = sender.item(selectedrow,0).text()
        print(self.deleteCommittee)
        if action == delete:
            msgBox = QtWidgets.QMessageBox(self)
            msgBox.setStyleSheet("background-color:  rgb(253, 253, 253);")
            msgBox.setIcon(QtWidgets.QMessageBox.Warning)
            msgBox.setText(f"Do you really want to delete {self.deleteCommittee}?")
            msgBox.setWindowTitle("Delete?")
            msgBox.setStandardButtons(QtWidgets.QMessageBox.Ok | QtWidgets.QMessageBox.Cancel)
            msgBox.buttonClicked.connect(partial(self.decision,selectedrow, sender))
            msgBox.show()
        
    def decision(self,selectedrow,sender,button):
        if button.text()=='OK':
            conn = sqlite3.connect(self.committeFilePath)
            cur = conn.cursor()
            cur.execute(f'DELETE from committee WHERE company_cin = "{self.CIN}" AND committee_name = "{self.deleteCommittee}"')
            cur.execute(f'DELETE from CommitteeMembers WHERE company_cin = "{self.CIN}" AND committee_name = "{self.deleteCommittee}"')
            cur.close()
            conn.commit()
            conn.close()
    ...
    def viewPolicyDoc(self):
        pass

    def openCommittee(self, ROW, COL):
        sender = self.sender()
        self.ActiveCommittee = sender.item(ROW,0).text()
        print(self.ActiveCommittee)
        self.addCommitte()
    ...
    def registerManager(self):
        ...
        Registers = {'MGT-1':'Register of Members','MGT-2': 'Register of Debenture holders',
                     'MGT-3': 'Foreign Register of Members','SH-2': 'Register of Renewed and Duplicate Share Certificate',
                     'SH-3': 'Register of Sweat Equity Shares','SH-6': 'Register of Employee Stock Options',
                     'SH-10': 'Register of Shares or Securities Bought Back',"Directors Register":"Register of Directors and KMPs",
                     "Deposits":"Register of Deposits",'CH-7': 'Register of Charges','MBP-2': "Register of Loans by Company",
                     "MBP-3": "Register of Investments not held in its own name","MBP-4": "Register of Contracts or Arrangements in which Directors are interested"}
        tlayout = 4
        positions = [(i, j) for i in range(int(len(Registers)+1))
                         for j in range(tlayout)]
        for position, Regname in zip(positions, Registers.keys()):
            Register = QtWidgets.QWidget()
            pyside_dynamic.loadUi('Resources/ui/Register.ui',Register)
            Register.registerName.setText(Regname)
            Register.registerdescrip.setText(Registers[Regname])
            Register.OpenButton.clicked.connect(self.OpenRegister)
            grid.addWidget(Register, *position)
        self.DisplayArea.layout().addWidget(Widget,*(0,0))
        

    def addCommitte(self):
        sender = self.sender()
        parent = getParent(sender,'HomePage')
        layout = parent.findChild(QtWidgets.QWidget, 'MainWindow').layout()
        clearLayout(layout)
        CurrentWidget = AddCommittee.Ui(self.CompanyBasicInfo[0], self.ActiveCommittee)
        layout.addWidget(CurrentWidget, *(0,0))
        
    def OpenRegister(self):
        sender = self.sender()
        parent = sender.parent().parent()
        RegisterName = parent.registerName.text().replace(' ','').replace('-','').upper()
        CurrentWidget = eval(RegisterName).Ui(self.CIN)
        clearLayout(self.DisplayArea.layout())
        self.DisplayArea.layout().addWidget(CurrentWidget,*(0,0))
...
"border-radius:10")
        ...
        self.ToolBoxWidget.currentChanged.connect(self.getMeetings)
        self.minutesPath = os.path.join(self.Config['Database'],'minutes.db')
        self.conn = sqlite3.connect(self.minutesPath)
        self.cur = self.conn.cursor()
        try:
            MeetYears = self.cur.execute(f'SELECT financial_year from minutes WHERE company_cin = "{self.CIN}"').fetchall()
        except sqlite3.OperationalError:
            MeetYears=[]
        if len(MeetYears)==0:
            year = self.findfinyear(datetime.datetime.now())
            ...
            meetYearsList = []
            for year in MeetYears:
                meetYearsList.append(year[0])
            meetYearsList = sorted(list(set(meetYearsList)),reverse=True)
            print(meetYearsList)
            for tabYear in meetYearsList:
                meetWidget = QtWidgets.QWidget()
                self.ToolBoxWidget.addItem(meetWidget, tabYear)
    ...
    def getMeetings(self, Index):
        Tab = self.sender()
        Year = Tab.itemText(Index)
        EventList = []
        try:
            self.minutesBook = self.cur.execute(f'SELECT * from minutes WHERE company_cin = "{self.CIN}" AND financial_year = "{Year}"').fetchall()
        except sqlite3.OperationalError:
            self.minutesBook = []
        print(self.minutesBook)
        meetWidget = Tab.widget(Index)
        pyside_dynamic.loadUi('Resources/ui/MeetingsManager.ui',meetWidget)
        meetWidget.meetingTable.setRowCount(len(self.minutesBook))
        meetWidget.meetingTable.setColumnWidth(0, 120)
        meetWidget.meetingTable.setColumnWidth(1, 120)
        meetWidget.meetingTable.setColumnWidth(2, 200)
        meetWidget.meetingTable.setColumnWidth(3, 200)
        ...
        if len(self.minutesBook) !=0:
            for x, minute in enumerate(self.minutesBook):
                for y in range(len(minute[:3])):
                    item = QtWidgets.QTableWidgetItem()
                    meetWidget.meetingTable.setItem(x, y, item)
                    item.setText(str(minute[y+1]))
                self.addTableButtons(meetWidget.meetingTable,x,minute[6],minute[7])
            meetWidget.meetingTable.sortItems(0, QtCore.Qt.AscendingOrder)
    ...
    def openDoc(self):
        ...
        button, docindex = senderBtn.split('_')
        ...
        if button == 'notice':
            ...
            docid = 7
        docName = self.segYeardata[year][int(docindex)][docid]
        print(docName)
        os.startfile(os.path.join(self.Config['Home'],docName)+'.docx')
    ...
    def addTableButtons(self,table,row, notice = '', minute=''):
        ...
        editNoticeButton.setObjectName(f"notice_{row}")
        ...
        table.setCellWidget(row, 3, NoFrame)
        ...
        addMinutesButton.clicked.connect(self.addMinute)
        ...
        table.setCellWidget(row, 4, MiFrame)
    ...
    def addMinute(self):
        date=None
        sender = self.sender()
        if sender.objectName() == 'addMinutesButton':
            meetingTable = sender.parent().parent().parent()
            print(meetingTable)
            index = meetingTable.indexAt(sender.parent().pos())
            print(index.row())
            date = meetingTable.item(index.row(),0).text()
            print(date)
        parent = getParent(sender,'HomePage')
        layout = parent.findChild(QtWidgets.QWidget, 'MainWindow').layout()
        clearLayout(layout)
        CurrentWidget = Minutes.Ui(self.CompanyBasicInfo[1], date)
        layout.addWidget(CurrentWidget, *(0,0))
    ...
    def Shareholders(self):
        if not self.CompanyShareholdersInfo[0][7]!='':
            ...
            try:
                ...
                QtWidgets.QMessageBox.critical(self, "File Deleted?", "The file you seek has been deleted. If you are Admin please add the document back to system or update.")
                print("File Deleted?")
    ...
    def Activities(self):
        ...
        ScrollArea.setWidget(meetWidget)
        ...
        meetWidget.meetingTable.setRowCount(len(activitiesData))
        meetWidget.meetingTable.setColumnWidth(0, 120)
        meetWidget.meetingTable.setColumnWidth(1, 120)
        ...
        for x, activity in enumerate(activitiesData):
            ...
            for y in range(len(filterList)):
                if y != 6:
                    ...
                    if filterList[y] !='':
                        ...
                        meetWidget.meetingTable.setCellWidget(x, y, ViewButton)
        meetWidget.meetingTable.sortItems(0, QtCore.Qt.AscendingOrder)
        ScrollArea.setWidgetResizable(True)
        self.DisplayArea.layout().addWidget(ScrollArea,*(0,0))
