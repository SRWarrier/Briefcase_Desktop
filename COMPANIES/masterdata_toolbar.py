...
class Ui(QtWidgets.QWidget):
    ...
    def captcha(self):
        sender =(self.sender())
        Home = sender.parent().parent().parent().parent().parent()
        print(Home)
        try:
    ...
    def getSignatories(self):
        ...
        if self.signatoriesinfo['Status']=='Failed':
            ...
            try:
                ...
                stylesheet = ["background-color: rgb(189,189,189)", "background-color: rgb(224,224,224)"]
                clearLayout(self.DirectorsDisplay.layout())
                for x in range(len(self.signatoriesdetails)):
                    DirectInfo = list(self.signatoriesdetails[x+1].values())
                    directorcard = QtWidgets.QWidget()
                    pyside_dynamic.loadUi('Resources/ui/DirectorInfoCard.ui',directorcard)
                    directorcard.setStyleSheet(stylesheet[x%2])
                    directorcard.DirectorInfo.setPlainText(DirectInfo[1]+'\n'+
                                                      DirectInfo[0]+'\n'+
                                                      DirectInfo[3]+'\n'+
                                                      DirectInfo[2])
                    self.DirectorsDisplay.layout().addWidget(directorcard)   
